                    ...
                    @forelse ($goodsReceipts as $gr)
                        <tr class="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="p-4 font-medium text-primary-600 dark:text-primary-400">
                                <button wire:click="viewDetail({{ $gr->id }})" class="hover:underline">{{ $gr->gr_number }}</button>
                            </td>
                            ...
                            <td class="p-4 text-right whitespace-nowrap">
                                @if ($gr->status === 'draft')
                                    <button wire:click="complete({{ $gr->id }})" wire:confirm="Konfirmasi penerimaan barang ini? Stok gudang akan diperbarui." class="p-2 text-slate-400 hover:text-success-600 transition-colors" title="Selesaikan">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                    </button>
                                    <button wire:click="triggerDelete({{ $gr->id }})" class="p-2 text-slate-400 hover:text-danger-600 transition-colors" title="Hapus">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                    </button>
                                @endif
                                <button wire:click="viewDetail({{ $gr->id }})" class="p-2 text-slate-400 hover:text-primary-600 transition-colors" title="Detail">
    ...
    <div x-data="{ show: @entangle('showModal') }" x-show="show" class="fixed inset-0 z-50 flex items-center justify-center" style="display: none;">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-3xl z-10 border border-slate-200 dark:border-slate-700 flex flex-col max-h-[90vh]">
                            ...
                            @foreach ($items as $index => $item)
                                <div class="grid grid-cols-12 gap-2 items-start bg-slate-50 dark:bg-slate-900/50 p-3 rounded-lg">
                                    <div class="col-span-4 pt-2 text-sm text-slate-800 dark:text-slate-200 font-medium">
                                        {{ $item['product_name'] }}
                                        <div class="text-xs text-slate-500">Dipesan: {{ $item['qty_ordered'] }}</div>
                                    </div>
            ...
            <div class="px-6 py-4 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3 bg-slate-50 dark:bg-slate-800/50 rounded-b-xl">
                ...
                <button wire:click="save" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-lg transition-colors shadow-lg shadow-primary-600/20">Simpan sebagai Draft</button>
            </div>
        </div>
    </div>
