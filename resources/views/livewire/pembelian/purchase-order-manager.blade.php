                    ...
                    @forelse ($purchaseOrders as $po)
                        <tr class="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="p-4 font-medium text-primary-600 dark:text-primary-400">
                                <button wire:click="viewDetail({{ $po->id }})" class="hover:underline">{{ $po->po_number }}</button>
                            </td>
                            ...
                            <td class="p-4 text-right whitespace-nowrap">
                                @if ($po->status === 'draft')
                                    <button wire:click="approve({{ $po->id }})" class="p-2 text-slate-400 hover:text-success-600 transition-colors" title="Setujui">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                    </button>
                                    <button wire:click="edit({{ $po->id }})" class="p-2 text-slate-400 hover:text-primary-600 transition-colors" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    <button wire:click="triggerDelete({{ $po->id }})" class="p-2 text-slate-400 hover:text-danger-600 transition-colors" title="Hapus">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                    </button>
                                @elseif (! in_array($po->status, ['closed', 'cancelled']))
                                    <button wire:click="cancel({{ $po->id }})" class="p-2 text-slate-400 hover:text-danger-600 transition-colors" title="Batalkan">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                    </button>
                                @endif
                                <button wire:click="viewDetail({{ $po->id }})" class="p-2 text-slate-400 hover:text-primary-600 transition-colors" title="Detail">
